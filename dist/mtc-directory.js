...
(function(){
  ...
  // Map chip label (uppercased category display name) to /directory category slug.
  var CHIP_TO_CATEGORY = {
    'CONDITION': 'condition', 'SYMPTOM': 'symptom', 'MEDICATION': 'medication',
    'SUPPLEMENT': 'supplement', 'LAB OR TEST': 'lab', 'PROCEDURE': 'procedure',
    'CONCEPT': 'concept'
  };
  // Plural display label for the breadcrumb.
  var CATEGORY_PLURAL = {
    condition: 'Conditions', symptom: 'Symptoms', medication: 'Medications',
    supplement: 'Supplements', lab: 'Labs & Tests', procedure: 'Procedures',
    concept: 'Concepts'
  };

  function injectCategoryCrumb(){
    var crumb = document.querySelector('.dir-detail__crumb');
    if (!crumb) return;
    if (crumb.querySelector('.mtc-crumb-cat')) return; // already injected
    var chipEl = document.querySelector('.dir-detail__chip-1, .dir-detail__chip');
    var chipText = chipEl ? (chipEl.textContent || '').trim().toUpperCase() : '';
    var slug = CHIP_TO_CATEGORY[chipText];
    if (!slug) return;
    ...
    link.textContent = CATEGORY_PLURAL[slug] || slug;
